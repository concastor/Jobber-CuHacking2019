...
function findChords(song){
  ...
  //create array
  let chords = []
  //nested for loop for each section of the song
  for(let i = 0; i < song.length; i++){
      //find beginig and end of chords
      let start = song[i].indexOf("[")
      let stop = song[i].indexOf("]")
      
      //add the chords to the array
      while(start !== -1 || stop !== -1){
          //adds to array
          chords.push(song[i].substring(start,stop+1))
          //should replace chord with black space
          song.replace(song[i].substring(start,stop+1),'')
          //find new begining and end
          start = song[i].indexOf("[",++stop)
          stop = song[i].indexOf("]", stop)
      }
  }
  return chords
}
...
function parseChordProFormat(chordProLinesArray) {
  ...
  for (let i = 0; i < chordProLinesArray.length; i++) {
    ...
    //     //find beginig and end of chords
    //     let start = chordLine[i].indexOf("[")
    //     let stop = chordLine[i].indexOf("]")
        
    //     //add the chords to the array
    //     while(start !== -1 || stop !== -1){
    //         //adds to array
    //         chords.push(chordLine[i].substring(start,stop+1))
    //         index.push(start)
    //         //should replace chord with black space
    //         lyricLine = lyricLine.replace(chordLine[i].substring(start,stop+1),'')
            

    //         //find new begining and end
    //         start = chordLine[i].indexOf("[",++stop)
    //         stop = chordLine[i].indexOf("]", stop)
    //       }
    //     }
